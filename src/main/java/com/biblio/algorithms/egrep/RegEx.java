...
public class RegEx {
    ...
    public static RegExTree parse() throws Exception {
        ...
        for (int i=0;i<regEx.length();i++)
            result.add(new RegExTree(
                    charToRoot(regEx.charAt(i)),new ArrayList<>()));
    ...
    private static RegExTree parse(ArrayList<RegExTree> result)
            throws Exception {
    ...
    private static ArrayList<RegExTree> processParenthese
            (ArrayList<RegExTree> trees) throws Exception {
    ...
    private static boolean containAltern(ArrayList<RegExTree> trees) {
        for (RegExTree t: trees)
            if (t.root==ALTERN && t.subTrees.isEmpty()) return true;
        return false;
    }
